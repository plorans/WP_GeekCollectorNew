...
add_filter('auctions_for_woocommerce_stripe_automatic_charge', 'auctions_for_woocommerce_stripe_automatic_charge_addfee', 5, 2);
...
add_filter('auctions_for_woocommerce_stripe_automatic_charge', function ($order, $auction_id) {
    ...
    if (empty($tokens)) {
        error_log('No Stripe tokens found');
        return $order;
    }
    ...
    $customer_id = get_user_meta($user_id, 'gc__stripe_customer_id', true);

    // error_log('Using PM: ' . $pm_id);
    // error_log('Customer ID: ' . $customer_id);

    if (!$customer_id) {
        error_log(' No Stripe customer ID found');
        return $order;
    }
    ...
    $order->update_meta_data('_stripe_payment_method', $pm_id);
    ...
    $_POST['wc-stripe-payment-method'] = $pm_id;
    $_POST['wc-stripe-payment-type'] = 'card';
